...
public:
   ...
   eosio_token_tester() {
      ...
      create_accounts( { N(alice), N(bob), N(carol), N(amax.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_stats( const string& symbolname )
   {
      ...
      vector<char> data = get_row_by_account( N(amax.token), name(symbol_code), N(stat), account_name(symbol_code) );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(amax.token), acc, N(accounts), account_name(symbol_code) );
